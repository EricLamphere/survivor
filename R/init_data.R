...
process_season_config <- function(config, people) {
    ...
    picks_tbl <- tibble::tibble(
        ...
        dplyr::select(season, dplyr::starts_with("person"), castaway_id)
...
process_seasons <- function(using = c("googlesheets", "configs"), 
    ...
    switch(
        using,
        googlesheets = .process_seasons__googlesheets(),
